...
def search():
    lb.delete(0, END)
    for row in database_account.search(firstname=firstname.get(), lastname=lastname.get(), username=username.get(), password=password.get(), country=country.get(), state=state.get(), date=date.get()):
        lb.insert(END, row)

def add():
    database_account.add(firstname.get(), lastname.get(), username.get(), password.get(), country.get(), state.get(), date.get())
    messagebox.showinfo("Add", "New Account Added Successfully")
    lb.delete(0, END)
    lb.insert(END, firstname.get(), lastname.get(), username.get(), password.get(), country.get(), state.get(), date.get())
...
def get_selected_row(event):
    try:
        ...
        EntryCoutry.delete(0,END)
        EntryCountry.insert(END,selected_tuple[5])

        EntryState.delete(0, END)
        EntryState.insert(END, selected_tuple[6])

        EntryDate.delete(0, END)
        EntryDate.insert(END, selected_tuple[7])
    except IndexError:
        pass

def update():
    database_account.update(selected_tuple[0], firstname.get(), lastname.get(), username.get(), password.get(), country.get(), state.get(), date.get())
...
lblcountry = Label(window,text="Country", font=("Calibri", 14, "bold"), fg="black", bg="navajo white")
lblcountry.grid(row=4,column=0,columnspan=2)

lblstate = Label(window,text="State", font=("Calibri", 14, "bold"), fg="black", bg="navajo white")
lblstate.grid(row=5,column=0,columnspan=2)

lbldate = Label(window,text="Date of entry", font=("Calibri", 14, "bold"), fg="black", bg="navajo white")
lbldate.grid(row=6,column=0,columnspan=2)
...
country=StringVar()
EntryCountry = Entry(window,textvariable=country, font=("Calibri", 14, "italic"), width=30)
EntryCountry.grid(row=4,column=0,columnspan=10)

state=StringVar()
EntryState = Entry(window,textvariable=state, font=("Calibri", 14, "italic"), width=30)
EntryState.grid(row=5,column=0,columnspan=10)

date=StringVar()
EntryDate = Entry(window,textvariable=date, font=("Calibri", 14, "italic"), width=30)
EntryDate.grid(row=6,column=0,columnspan=10)
...
ViewAllButton.grid(row=7,column=3)
...
ClearAllButton.grid(row=7,column=5)
